...
class ECClassifier(nn.Module):
    ...
    def __init__(self, embeddings, pos_embedding, opt):
        super().__init__()
        self.gcn_model = FSSGCN(embeddings, pos_embedding, opt)
        in_dim = 2 * opt.hidden_dim #200
        self.classifier = nn.Linear(in_dim, opt.num_class) #binary classification
        self.opt = opt

    def forward(self, inputs):
        outputs = self.gcn_model(inputs) #(32, 75, 200)
        logits = self.classifier(outputs)
        return logits

class FSSGCN(nn.Module):
    def __init__(self, embeddings, pos_embedding, opt):
        super().__init__()
        self.opt = opt
        self.in_dim = opt.embed_dim
        self.emb, self.pos_emb = nn.Embedding.from_pretrained(torch.tensor(embeddings, dtype=torch.float)).to(self.opt.device), \
                                 torch.nn.Embedding(self.opt.pos_num, self.opt.embedding_dim_pos)
                                 # nn.Embedding.from_pretrained(torch.tensor(pos_embedding, dtype=torch.float)).to(self.opt.device)
        ...
        if self.opt.no_rnn == False:
            ...
            self.rnn = DynamicLSTM(opt.embed_dim, opt.rnn_hidden, num_layers=opt.rnn_layer, batch_first=True, bidirectional=True)#(32,75,45,200)
        ...
        self.multi_attn = MultiHeadAttention(self.heads, self.mem_dim)
        self.single_attn = SingleHeadAttention(self.mem_dim)
    ...
    def forward(self, inputs):
        ...
        word_embs = torch.reshape(self.emb(words), [-1, words.shape[-1], 2*self.opt.hidden_dim]) #(32*75, 45, 200)
        if self.opt.no_rnn == False:
        ...
        layer_list = []
        adj_rep = self.single_attn(gcn_inputs, gcn_inputs)
        outputs = gcn_inputs
        for i in range(len(self.layers)):
            if i < 2:
                outputs = self.layers[i](adj*adj_rep, outputs)
                layer_list.append(outputs)
            else:
                attn_tensor = self.multi_attn(outputs, outputs)
                attn_adj_list = [attn_adj.squeeze(1) for attn_adj in torch.split(attn_tensor, 1, dim=1)]
                outputs = self.layers[i](attn_adj_list, adj, outputs)
                layer_list.append(outputs)
...
class MultiGraphConvLayer(nn.Module):
    ...
    def forward(self, adj_list, adj_syn, gcn_inputs):

        multi_head_list = []
        for i in range(self.heads):
            adj = adj_list[i] * adj_syn
            denom = adj.sum(2).unsqueeze(2) + 1
            outputs = gcn_inputs
            cache_list = [outputs]
            output_list = []
...
class SingleHeadAttention(nn.Module):
    ...
    def forward(self, query, key, mask=None):
        if mask is not None:
            mask = mask.unsqueeze(1)
        query, key = self.linears(query), self.linears(key)
        attn = attention(query, key, mask=mask, dropout=self.dropout)
